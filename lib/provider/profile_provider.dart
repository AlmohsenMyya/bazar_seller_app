...
class ProfileProvider extends ChangeNotifier {
  ...
  void removeAddressById(int? id, int index, BuildContext context) async {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      initAddressList();
      Provider.of<OrderProvider>(Get.context!, listen: false).shippingAddressNull();
      Provider.of<OrderProvider>(Get.context!, listen: false).billingAddressNull();
  ...
  // edit by almohsen
  Future<UserInfoModel?> getUserAllInfo(BuildContext context) async {
    String userID = '-1';
    ApiResponse apiResponse = await profileRepo!.getUserInfo();
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      _userInfoModel = UserInfoModel.fromJson(apiResponse.response!.data);
  ...
  Future<ResponseModel> updateUserInfo(UserInfoModel updateUserModel, String pass, File? file, String token) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (kDebugMode) {
        print('almohsen ** ${response.statusCode} ${response.reasonPhrase}');
      }
  ...
  // edite by almohsen
  Future<void> updateIsActive(bool isActive, String token) async {
    http.Response response = await profileRepo!.updateIsActive(isActive, token);
    if (response.statusCode == 200){
      print("finally");
      print(response.body);
